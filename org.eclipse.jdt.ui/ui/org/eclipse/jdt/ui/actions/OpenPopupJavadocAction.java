...
/**
 * This action opens a popup Javadoc browser on a Java element or file.
 * <p>
 ...
 */
public class OpenPopupJavadocAction extends SelectionDispatchAction {
	...
	/**
	 * Creates a new <code>OpenPopupJavadocAction</code>. The action requires that the selection
	 ...
	 */
	public OpenPopupJavadocAction(IWorkbenchSite site) {
		super(site);
		setText(ActionMessages.OpenPopupJavadocAction_label);
		setDescription(ActionMessages.OpenPopupJavadocAction_description);
		setToolTipText(ActionMessages.OpenPopupJavadocAction_tooltip);
		PlatformUI.getWorkbench().getHelpSystem().setHelp(this, IJavaHelpContextIds.OPEN_POPUP_JAVADOC_ACTION);
	}
	 ...
	 */
	public OpenPopupJavadocAction(JavaEditor editor) {
	...
	public void run(ITextSelection selection) {
		...
		try {
			...
			if (elements.length > 1) {
				candidate= SelectionConverter.selectJavaElement(elements, getShell(), getDialogTitle(), ActionMessages.OpenPopupJavadocAction_select_element);
				}
			if (candidate != null) {
				run(candidate);
				}
		} catch (InvocationTargetException e) {
			ExceptionHandler.handle(e, getShell(), getDialogTitle(), ActionMessages.OpenPopupJavadocAction_code_resolve_failed);
		...
		try {
			...
			if (baseURL == null) {
				...
				if (root != null && root.getKind() == IPackageFragmentRoot.K_BINARY) {
					String message= ActionMessages.OpenPopupJavadocAction_libraries_no_location;
					...
					String message= ActionMessages.OpenPopupJavadocAction_source_no_location;
			...
			if (url != null)
				...
				JavaPlugin.log(e);
			showMessage(shell, ActionMessages.OpenPopupJavadocAction_opening_failed, true);
			}
		}
	...
	protected void open(URL url) {
		if (forceExternalBrowser())
			...
			EModelService service= PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().getActiveEditor().getSite().getService(EModelService.class);
			MPartSashContainerElement mPart= PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().getActiveEditor().getSite().getService(MPart.class);
	...
	private static String getTitle() {
		return ActionMessages.OpenPopupJavadocAction_dialog_title;
		}
